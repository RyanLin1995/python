...
class Example121SpiderMiddleware:
...
class Example121DownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class Example121ProxyMiddleware:  # 编写一个自动换代理的中间件
...
class RetryOfDateMiddleware(RetryMiddleware):
    ...
    def process_exception(self, request, exception, spider):
        if spider.name == 'exceptionSpider' and isinstance(exception, TCPTimedOutError):
            ...
            return request.copy()  # 返回这个请求对象request。返回以后，Scrapy会重新调度这个请求，就像它第一次调度一样。
...
class ExceptionCheckSpider:

    def process_spider_exception(self, response, exception, spider):
        if spider.name == 'example12_1_2':
