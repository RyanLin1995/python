...
def check_events(ship, ai_settings, screen, bullets, stats, button, aliens, sb):
    """响应鼠标和键盘"""
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_button(stats, button, mouce_x, mouce_y, bullets, ai_settings, screen, ship, aliens, sb)
...
def check_button(stats, button, mouce_x, mouce_y, bullets, ai_settings, screen, ship, aliens, sb):
    """在玩家单击Play按钮时开始新游戏"""
    button_click = button.rect.collidepoint(mouce_x, mouce_y)
...
def update_bullets(ai_settings, screen, ship, aliens, bullets, stats, sb):
    ...
    check_bullets_alien_collisions(ai_settings, screen, ship, aliens, bullets, stats, sb)
...
def check_bullets_alien_collisions(ai_settings, screen, ship, aliens, bullets, stats, sb):
    """响应子弹和外星人的碰撞"""
    # 删除发生碰撞的子弹和外星人
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    for rows_number in range(number_rows):
        for alien_number in range(number_alien_x):
            create_alien(ai_settings, screen, aliens, alien_number, rows_number)
...
def update_screen(ai_setting, screen, ship, bullets, aliens, button, stats, sb):
    """更新屏幕上的图像，并切换到新屏幕"""
    #  每次循环时都重绘屏幕
