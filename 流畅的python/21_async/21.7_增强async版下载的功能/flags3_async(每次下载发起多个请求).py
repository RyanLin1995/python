...
async def get_country(client: httpx.AsyncClient,
    ...
    resp.raise_for_status()
    metadata = resp.json()  # metadata 是一个 python 字典，根据响应的 JSON 内容构建
    return metadata['country']  # 返回国家名称
...
async def download_one(client: httpx.AsyncClient,
    ...
    try:
        async with semaphore:  # 持有 semaphore，异步等待 get_flag
            image = await get_flag(client, base_url, cc)
        async with semaphore:  # 持有 semaphore，异步等待 get_country
        ...
        filename = country.replace(' ', '_')  # 使用返回的国家名称创建用户名
