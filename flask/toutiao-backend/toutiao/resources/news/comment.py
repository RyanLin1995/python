...
class CommentListResource(Resource):
    ...
    def post(self):
        ...
        if not article_id:
            ...
            comment_id = current_app.id_worker.get_id()
            comment = Comment(id=comment_id, user_id=g.user_id, article_id=article_id, parent_id=target,
                              content=content)
            db.session.add(comment)
            db.session.commit()
    ...
    def get(self):
        ...
        if args.type == 'a':
            ...
            total_count, end_id, last_id, ret = cache_comment.ArticleCommentsCache(article_id).get_page(args.offset,
                                                                                                        limit)
        else:
            # 评论的评论
